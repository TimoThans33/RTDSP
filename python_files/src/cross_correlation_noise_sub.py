...
audData_floor11, rate = librosa.core.load("../SoundSamples/noisy_journey_8k.wav", sr = None)
audData_beep, rate = librosa.core.load("../SoundSamples/noisy_journey_8k.wav", sr = None)
sampData_noise, rate = librosa.core.load("../SoundSamples/crowd-talking-1-short.wav", sr = 8000)
sampData_floor11, rate = librosa.core.load("../SoundSamples/eleven_8k_short.wav", sr = None)
sampData_beep, rate = librosa.core.load("../SoundSamples/beep_8k.wav", sr = None)

audtime_floor11 = np.arange(0, len(audData_floor11), 1)
audtime_beep = np.arange(0, len(audData_beep), 1)
samptime_noise = np.arange(0, len(sampData_noise[0:len(sampData_floor11)-1]))
samptime_floor11 = np.arange(0, len(sampData_floor11), 1)
samptime_beep = np.arange(0, len(sampData_beep), 1)

freq_data_samp_noise = fft(sampData_noise[100:len(sampData_beep)+99])
freq_data_samp_floor11 = fft(sampData_floor11)
freq_data_samp_beep = fft(sampData_beep)

X_sample_noise = np.zeros([len(samptime_beep)])
X_sample_floor11 = np.zeros([len(samptime_floor11)])
X_sample_beep = np.zeros([len(samptime_beep)])

X_sample_noise[0:np.int(len(samptime_beep)/2)] = 2/len(samptime_beep)*np.abs(freq_data_samp_noise[0:np.int(len(samptime_beep)/2)])
...
plt.figure(0)
plt.plot(X_sample_noise)
plt.plot(X_sample_beep)
plt.show()
...
WINDOW_SIZE_BEEP = 500

"""
Define here your minimum values
"""
MIN_VALUE_FLOOR11 = 0.006

MIN_VALUE_BEEP = 0.0002
    ...
    while g <= len(audtime_beep)-1:
        ...
        if counter > WINDOW_SIZE_BEEP:
            ...
            for k in range(len(buffer)):
                ...
                array[g-WINDOW_SIZE_BEEP-1:g] -= Xjw_buf[k]*X_sample_noise[k]/30
            counter=0
